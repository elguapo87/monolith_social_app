...
import { getConnectioUser } from "@/redux/slices/connectionSlice";
...
const ChatBox = () => {
    ...
    useEffect(() => {
        getToken().then((token) => {
            dispatch(getConnectioUser({ otherUserId: userId, token })).unwrap();
            dispatch(resetMessages());
            dispatch(fetchMessages({ to_user_id: userId, token })).unwrap();
        });
                ...
                <div className="space-y-4 max-w-4xl mx-auto">
                    {messages.toSorted((a, b) =>
                        ...
                        .map((message, index) => {
                            ...
                            const isMine =
                                getUserId(message.from_user_id) === currentUser?._id;
                            ...
                            return (
                                ...
                                >
                                    <div
                                        ...
                                        shadow ${!isMine ? "rounded-bl-none" : "rounded-br-none"}`}
                                    >
